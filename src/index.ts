...
export class OpenAIVoice extends MastraVoice {
  ...
  constructor({
    ...
    this.speechClient = new OpenAI({
      apiKey: speechApiKey,
      baseURL: speechModel?.baseUrl || defaultBaseurl,
    });
    ...
    this.listeningClient = new OpenAI({
      ...
      baseURL: listeningModel?.baseUrl || defaultBaseurl,
    });

    if (!this.speechClient && !this.listeningClient) {
      throw new Error(
        'At least one of OPENAI_API_KEY, speechModel.apiKey, or listeningModel.apiKey must be set'
      );
    }
  }
